...
export default function App() {
    ...
    const [createAdSetsPaused, setCreateAdSetsPaused] = useState(true);
    const [createAdsPaused, setCreateAdsPaused] = useState(true);
    ...
    const [reactionWorkerConcurrency, setReactionWorkerConcurrency] = useState(5);
    const [reactionWorkerProxyIds, setReactionWorkerProxyIds] = useState({});
    const [reactionBrowserMode, setReactionBrowserMode] = useState("visible");
    const [reactionDisableImages, setReactionDisableImages] = useState(false);
    const [apiClientsBrowserMode, setApiClientsBrowserMode] = useState("visible");
    const [apiClientsDisableImages, setApiClientsDisableImages] = useState(false);
    ...
    useEffect(() => {
        ...
        const offTasks = window.adsBot.onBackgroundTasksUpdated((event) => {
            ...
            if (event.type === "updated" && ["completed", "completed_with_warnings"].includes(event.task?.status)) {
                if (event.task.type === "facebook-api-client-sync") {
                    setAccounts((current) => current.map((account) => (
                        account.accountKey === event.task.result?.accountKey
                            ? { ...account, hasUserAgent: true, hasAccessToken: true, hasCookie: true }
                            : account
                    )));
                }
        ...
        const initialize = async () => {
            try {
                ...
                setUiScale(state.uiScale); setCreateCampaignsPaused(state.createCampaignsPaused); setCreateAdSetsPaused(state.createAdSetsPaused); setCreateAdsPaused(state.createAdsPaused); setCommentWorkerConcurrency(state.commentWorkerConcurrency);
                ...
                setReactionWorkerConcurrency(state.reactionWorkerConcurrency ?? 5);
                setReactionWorkerProxyIds(state.reactionWorkerProxyIds || {});
                setReactionBrowserMode(state.reactionBrowserMode || "visible");
                setReactionDisableImages(state.reactionDisableImages === true);
                setApiClientsBrowserMode(state.apiClientsBrowserMode || "visible");
                setApiClientsDisableImages(state.apiClientsDisableImages === true);
    ...
    const updateAccount = async (key, patch) => {
        ...
        setAccounts((current) => current.map((account) => (
            account.accountKey === key ? { ...account, ...updated } : account
        )));
        setWorkspaceCache((current) => { const next = { ...current }; delete next[key]; return next; });
        showToast("API-клієнта оновлено", "success");
    };
    ...
    const syncAccount = async (key) => {
        const result = await unwrap(window.adsBot.syncAccountFromAdsPower(key, {
            browserMode: apiClientsBrowserMode,
            disableImages: apiClientsDisableImages,
        }));
        setTasks((current) => [result.task, ...current.filter((task) => task.id !== result.task.id)]);
        showToast("Синхронізацію API-клієнта додано до черги", "success");
    };
    const syncingApiClientKeys = tasks
                ...
                {activeTab === "comment-accounts" && (
                    <CommentAccountsTab
                        ...
                        settings={{
                            ...
                            reactionWorkerConcurrency,
                            reactionWorkerProxyIds,
                            reactionBrowserMode,
                            reactionDisableImages,
                        }}
                    />
                )}
                {activeTab === "keitaro" && (
