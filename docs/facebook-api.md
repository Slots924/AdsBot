...
`publishPost()` приймає текст і `imagePaths` — упорядкований масив локальних
зображень (JPG, JPEG, PNG або WEBP). Одне фото також можна передати через
застарілий сумісний параметр `imagePath`. Для двох і більше фото Graph API
спочатку завантажує їх непублічно, а потім створює один пост через
`attached_media` у переданому порядку.
...
Для сумісності з актуальною версією Graph API snapshot постів більше не
запитує застаріле поле `object_id`; для видалення використовуються власні ID
постів і фотографій.
