...
5. Початковий HTTP-запит повторюється один раз.
...
Для майбутніх POST та інших методів доступний універсальний `request(config)`. Передані викликачем `httpAgent`, `httpsAgent` і стандартна Axios proxy-конфігурація завжди замінюються агентом активної проксі.
